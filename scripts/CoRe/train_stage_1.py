...
def log_validation(
    ...
    for prompt in validation_prompt_set:
        imgs = pipeline(
            prompt.format(f'{args.placeholder_token}'),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        help="Flag to add embedding regularization."
    )
    parser.add_argument(
        "--add_attn_reg",
        type=lambda x: x.lower() in ("true", "1"),  # Accepts 'True', '1', 'False', '0'
    ...
    parser.add_argument(
        ...
        default=False,
        help="Calculate gram matrix of cross-attn maps or not. Accepts 'True'/'1' or 'False'/'0'."
    )
...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        class_text = p.format(superclass)
        concept_text = p.format(placeholder_string) 
        instance_text = 'a photo of a {0}'.format(placeholder_string)

        concept_text_words = concept_text.split()
        if placeholder_string not in concept_text_words:
            print(f"Placeholder string '{placeholder_string}' not found in text2_words: {concept_text_words}")
            raise ValueError(f"'{placeholder_string}' is not in list")

        word_id = concept_text_words.index(placeholder_string)

        if concept_text_words[word_id] != placeholder_string:
            print("Error: text2_words[word_id] is not equal to placeholder_string")
            print(f"word_id = {word_id}, text2[word_id] = concept_text_words[word_id]")
        ...
        example["class_text_1"] = self.tokenizer_1(
            class_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer_1.model_max_length,
        ...
        example["class_text_2"] = self.tokenizer_2(
            class_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer_2.model_max_length,
        ...
        example["concept_text_1"] = self.tokenizer_1(
            concept_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer_1.model_max_length,
        ...
        example["concept_text_2"] = self.tokenizer_2(
            concept_text,
            padding="max_length",
            truncation=True,
            max_length=self.tokenizer_2.model_max_length,
        ...
        # example["concept_word_id_1"] = torch.argwhere(torch.eq(example["concept_text_1"], self.tokenizer_1.encode(placeholder_string, add_special_tokens=False)[0])).item()
        # example["concept_word_id_2"] = torch.argwhere(torch.eq(example["concept_text_2"], self.tokenizer_2.encode(placeholder_string, add_special_tokens=False)[0])).item()
        # example["concept_word_id_1"] = define_target_token_id(self.tokenizer_1, placeholder_string, concept_text)
        # example["concept_word_id_2"] = define_target_token_id(self.tokenizer_2, placeholder_string, concept_text)
...
def main():
    ...
    class_token_ids = tokenizer_1.encode(args.class_name, add_special_tokens=False)
    class_token_ids_2 = tokenizer_2.encode(args.class_name, add_special_tokens=False)
    ...
    optimizer = optimizer_class(
        # text_encoder_1.get_input_embeddings().parameters(),  
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("CoRe_stage1_thesis", config=vars(args), init_kwargs={"wandb": {"name": args.wandb_exp_name}})
    ...
    superclass_embedding = (
        accelerator.unwrap_model(text_encoder_1)
        .get_input_embeddings()
        .weight[class_token_ids]
    ).detach().cpu()

    superclass_embedding_2 = (
        accelerator.unwrap_model(text_encoder_2)
        .get_input_embeddings()
        .weight[class_token_ids_2]
    ).detach().cpu()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate([text_encoder_1, text_encoder_2]):
                ...
                # Class text
                class_encoder_hidden_states_1 = (
                    text_encoder_1(batch["class_text_1"], output_hidden_states=True)
                    .hidden_states[-2]
                    .to(dtype=weight_dtype)
                )
                class_encoder_output_2 = text_encoder_2(batch["class_text_2"], output_hidden_states=True)
                class_encoder_hidden_states_2 = class_encoder_output_2.hidden_states[-2].to(dtype=weight_dtype)
                class_encoder_hidden_states = torch.cat([class_encoder_hidden_states_1, class_encoder_hidden_states_2], dim=-1)
                class_added_cond_kwargs = {"text_embeds": class_encoder_output_2[0], "time_ids": add_time_ids}


                # Concept text
                concept_encoder_hidden_states_1 = (
                    text_encoder_1(batch["concept_text_1"], output_hidden_states=True)
                    .hidden_states[-2]
                    .to(dtype=weight_dtype)
                )
                concept_encoder_output_2 = text_encoder_2(batch["concept_text_2"], output_hidden_states=True)
                concept_encoder_hidden_states_2 = concept_encoder_output_2.hidden_states[-2].to(dtype=weight_dtype)
                concept_encoder_hidden_states = torch.cat([concept_encoder_hidden_states_1, concept_encoder_hidden_states_2], dim=-1)
                concept_added_cond_kwargs = {"text_embeds": concept_encoder_output_2[0], "time_ids": add_time_ids}

                word_ids = [torch.argmax(batch["concept_text_1"][i]).item() for i in range(bsz)]
                
                # Context embedding loss
                loss_emb = 0
                for i in range(bsz):
                    word_id = word_ids[i]
                    clean_class_encoder_hidden_states = class_encoder_hidden_states.clone()
                    clean_concept_encoder_hidden_states = concept_encoder_hidden_states.clone()
                    clean_class_encoder_hidden_states[i, word_id, :] = 0
                    clean_concept_encoder_hidden_states[i, word_id, :] = 0

                cos_sim = F.cosine_similarity(
                    clean_class_encoder_hidden_states, 
                    clean_concept_encoder_hidden_states, 
                    dim=-1
                ) 
                loss_emb += torch.sum(1 - cos_sim) / (clean_class_encoder_hidden_states.shape[1]-1)
                
                
                # Context attention loss
                loss_attn = 0

                # Generate attention for class_encoder_hidden_states 
                ...
                attention_probs_list1 = get_cross_attnention_maps(save_output, unet)
                interp_attention_maps1 = interpolate_cross_attention_maps(attention_probs_list1, with_gram=args.with_gram, min_seq=False)
                mean_attn_map1 = interp_attention_maps1.mean(dim=0)
                
                for i in range(bsz):
                    word_id = word_ids[i]
                    if args.with_gram:
                        mean_attn_map1[word_id, word_id] = 0
                    else:
                ...
                # Generate attention for concept_encoder_hidden_states 
                save_output = SaveOutput()
                hook_handles = []
                for name, layer in unet.named_modules():
                    if 'attn2.to_q' in name or 'attn2.to_k' in name:
                ...
                # Model prediction for concept
                _ = unet(
                    noisy_latents, 
                    timesteps, 
                    concept_encoder_hidden_states,
                    added_cond_kwargs=concept_added_cond_kwargs
                )

                attention_probs_list2 = get_cross_attnention_maps(save_output, unet)
                interp_attention_maps2 = interpolate_cross_attention_maps(attention_probs_list2, with_gram=args.with_gram, min_seq=False)
                mean_attn_map2 = interp_attention_maps2.mean(dim=0)

                for i in range(bsz):
                    word_id = word_ids[i]
                    if args.with_gram:
                        mean_attn_map2[word_id, word_id] = 0
                    else:
                ...
                if args.add_attn_reg:
                    if args.with_gram:
                        loss_attn = torch.sqrt(loss_attn)
                ...
                v_prev = accelerator.unwrap_model(text_encoder_1).get_input_embeddings().weight[-1] 
                v_prev_2 = accelerator.unwrap_model(text_encoder_2).get_input_embeddings().weight[-1] 
                ...
                v_curr = accelerator.unwrap_model(text_encoder_1).get_input_embeddings().weight[-1] 
                v_curr = v_curr / torch.norm(v_curr, dim=-1, keepdim=True) * torch.norm(v_prev, dim=-1, keepdim=True)
                v_curr_2 = accelerator.unwrap_model(text_encoder_2).get_input_embeddings().weight[-1] 
                v_curr_2 = v_curr_2 / torch.norm(v_curr_2, dim=-1, keepdim=True) * torch.norm(v_prev_2, dim=-1, keepdim=True)

                cos_sim_between_cls_plh_embeddings = F.cosine_similarity(superclass_embedding, v_curr.cpu().unsqueeze(0))
                cos_sim_between_cls_plh_embeddings_2 = F.cosine_similarity(superclass_embedding_2, v_curr_2.cpu().unsqueeze(0))
                
                if step >= 120 and step<=180:
                    orig_embeds_params[-1] = v_curr 
                if step >= 120 and step<=180:
                    orig_embeds_params_2[-1] = v_curr_2
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    save_path = Path(os.path.join(args.output_dir, 'stage_1', f"checkpoint-{global_step}"))
                ...
                if accelerator.is_main_process:
                    if global_step == 0 or global_step % args.checkpointing_steps == 0:
                        ...
                        save_path = os.path.join(args.output_dir, 'stage_1', f"checkpoint-{global_step}")
            ...
            logs = {
                ...
                f"loss_emb_lambda{args.lambda_emb}": (args.lambda_emb*loss_emb).detach().item(),
                f"loss_attn_lambda{args.lambda_attn}": (args.lambda_attn*loss_attn).detach().item(),
                "loss_emb": loss_emb.detach().item(),
                "loss_attn": loss_attn.detach().item(), 
                "lr": lr_scheduler.get_last_lr()[0], 
                "cos_sim_bw_csl_plh_embeddings_1": cos_sim_between_cls_plh_embeddings.detach().item(),
                "cos_sim_bw_csl_plh_embeddings_2": cos_sim_between_cls_plh_embeddings_2.detach().item(),
            }
    ...
    save_path = os.path.join(args.output_dir, "text_encoder_1.bin")
    torch.save(accelerator.unwrap_model(text_encoder_1).state_dict(), save_path)
    print(f"Text encoder saved to {save_path}")
    
    save_path = os.path.join(args.output_dir, "text_encoder_2.bin")
    torch.save(text_encoder_2.state_dict(), save_path)
    print(f"Text encoder saved to {save_path}")
