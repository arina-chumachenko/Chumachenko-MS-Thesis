...
def parse_args():
    ...
    parser.add_argument(
        "--use_noft_unet_for_cls",
        type=lambda x: x.lower() in ("true", "1"),  # Accepts 'True', '1', 'False', '0'
        default=False,
        help="Flad to use nofine-tuned unet for class attention maps in an attention regularization."
    )
    parser.add_argument(
        "--with_gram",
...
def main():
    ...
    initializer_token_id = tokenizer_1.convert_tokens_to_ids([args.initializer_token])[0]
    initializer_token_id_2 = tokenizer_2.convert_tokens_to_ids([args.initializer_token])[0]
    ...
    # load text encoder from first stage here
    state_dict_1 = torch.load(os.path.join(args.output_dir, "text_encoder_1.bin"))
    text_encoder_1.load_state_dict(state_dict_1)
    state_dict_2 = torch.load(os.path.join(args.output_dir, "text_encoder_2.bin"))
    text_encoder_2.load_state_dict(state_dict_2)

    # Freeze vae and text encoders
    vae.requires_grad_(False)
    unet_orig.requires_grad_(False)
    # unet.requires_grad_(False)
    for name, layer in unet.named_modules():
        layer.module_name = name
    for name, layer in unet_orig.named_modules():
        layer.module_name = name
    ...
    parameters_to_optimize =(
        # itertools.chain(unet.parameters(), text_encoder_1.parameters(), text_encoder_2.parameters()) if args.train_text_encoder else 
        unet.parameters()
    )
    ...
    unet_orig.to(accelerator.device, dtype=weight_dtype)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("CoRe_stage2_thesis", config=vars(args), init_kwargs={"wandb": {"name": args.wandb_exp_name}})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate([text_encoder_1, text_encoder_2]):
                ...
                attention_probs_list1 = get_cross_attnention_maps(save_output, model)
                interp_attention_maps1 = interpolate_cross_attention_maps(attention_probs_list1, with_gram=args.with_gram)
                ...
                interp_attention_maps2 = interpolate_cross_attention_maps(attention_probs_list2, with_gram=args.with_gram)
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        images = log_validation(
                            unwrap_model(text_encoder_1) if text_encoder_1 is not None else text_encoder_1,
                            unwrap_model(text_encoder_2) if text_encoder_2 is not None else text_encoder_2,
                            tokenizer_1,
                            tokenizer_2,
                            unwrap_model(unet),
                            vae,
    ...
    if accelerator.is_main_process:
        ...
        if text_encoder_1 is not None:
            pipeline_args["text_encoder"] = unwrap_model(text_encoder_1)
