...
class DINOEvaluator(CLIPEvaluator):
    ...
    @torch.no_grad()
    def dino_encode_images(self, images) -> torch.Tensor:
        images = torch.stack([self.dino_preprocess(image) for image in images])
        images = images.to(self.device)
        return self.dino_model(images)

    def get_dino_image_features(self, img, norm: bool = True) -> torch.Tensor:
...
def narrow_similarities(
        text_similarities, prompts, holder, verbose: bool = False
):
    # (text_similarities: Dict[str, float], prompts, holder: Optional[str], verbose: bool = False) -> Tuple[Optional[float], Optional[float]]:
